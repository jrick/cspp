...
const (
	...
	// inverse timeouts compared to clients
...
// recvTimeout returns receiving deadlines that exponentially increases with the
// total number of mix clients for large messages.
/*
func (s *session) recvTimeout(large bool) time.Duration {
	timeout := 10 * time.Second
	for i := 0; large && i < (len(s.clients)-5)/5; i++ {
		timeout += time.Duration(i+1) * 500 * time.Millisecond
	}
	return timeout
}

func (s *session) recvDeadline(large bool) time.Time {
	return time.Now().Add(s.recvTimeout(large))
}

func (s *session) sendTimeout(large bool) time.Duration {
...
func (s *session) sendDeadline(large bool) time.Time {
	return time.Now().Add(s.sendTimeout(large))
}
*/
...
func (c *client) run(ctx context.Context, run int, s *session, ke *messages.KE) error {
	...
	case <-ctx.Done():
		return ctx.Err()
	case <-blaming:
		// XXX: RS deadline must be set by session before signaling blaming
...
func (c *client) blame(ctx context.Context, s *session, run int) error {
	rs := new(messages.RS)
	// XXX: set a read deadline (from the session?). disable for now.
	...
	case <-ctx.Done():
		return ctx.Err()
	case msg := <-c.out:
		// XXX: set a send deadline (from the session?). disable for now.
